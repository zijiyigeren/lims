<template>
  <section class="lxt_bgcolor">
    ...
    <div class="lxt_pubit" v-if="hiddenFlg">
      <el-select class="lxt_distribution" v-model="Categoryvalue" placeholder="请选择" @change="select_type">
        <el-option
      ...
      <el-select class="lxt_date" v-model="selectDateValue" placeholder="请选择" @change="select_date">
        <el-option
      ...
      <el-input autocomplete="off" placeholder="请输入内容"
      ...
      <div class="lxt_fire">最近的搜索</div>
      <div class="lxt_fireBtns">
        <el-button plain v-for="(item,index) in fireBtn" :key="index"
    ...
    <!-- table表格：分配检测 -->
    <div style="background: #ffffff;margin: 10px;padding-bottom:5px;" v-if="hiddenFlg">
      <div class="commonModule-title">
        <i class="commonModule-i"></i>
        搜索内容 ：{{arrSearchType | searchType }}
      </div>
      <tablepage :isTableType="isTableType" :arrsearch='arrsearch' :isLoading="child_loading"
    ...
    methods: {
      ...
      * */
      setNearlyCondition(params) {
        ...
        this.$api.systemPublicationpage.setNearlyCondition(params).then(res => {
      ...
      * */
      getNearlyCondition() {
        this.$api.systemPublicationpage.getNearlyCondition().then(res => {
          // console.log(res);
          if (res.data.ok && res.data.data) {
            this.fireBtn = res.data.data
          }
      ...
      * */
      getUsualSearch() {
        this.$api.systemPublicationpage.getUsualSearch().then(res => {
          if (res.data.ok && res.data.data) {
