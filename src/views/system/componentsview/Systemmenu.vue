    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form" label-width="80px" ref="menuForm" :rules="rules1">
        ...
        <el-form-item label="菜单等级" prop="menuLevel">
          <el-select v-model="form.menuLevel" placeholder="请选择菜单等级">
            <el-option label="一级菜单" value="level1"></el-option>
            <el-option label="二级菜单" value="level2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="父级菜单" v-if="form.menuLevel === 'level2'">
        ...
        <el-form-item label="菜单地址" v-if="form.menuLevel === 'level2'" prop="menuAddress">
          <el-input placeholder="菜单地址" v-model="form.menuAddress"></el-input>
        </el-form-item>
...
<script>
  export default {
    ...
    methods: {
      // 查询菜单列表
      findList() {
        this.$api.menu.getMenuList().then(resp => {
      ...
      save(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.form.menuId) {
              this.$api.menu.editMenu(this.form).then(resp => {
                this.$message({
              ...
              this.$api.menu.addMenu(this.form).then(resp => {
                this.$message({
                  message: resp.data.data,
                  type: "success"
                });
      ...
      // 删除菜单
      deleteMenu(row) {
        this.$confirm("是否确认删除所选菜单？", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.$api.menu.deleteMenu({
            menuLevel: row.menu1 ? "level2" : "level1",
            menuId: row.id
          }).then(resp => {
            this.$message({
              message: resp.data.data,
