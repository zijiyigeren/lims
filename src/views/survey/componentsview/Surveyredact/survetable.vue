<template>
  <section class="dislogBox">
    ...
    <div class="componentBoxs">
      ...
      <!-- 一级列表 -->
      <div class="Interone" ref="interone">
        <div class="cB_assembly" v-for="(item,index) in dataList" :key="index">
          ...
          <div class="oneFirstFloor" id="oneFirstFloor" v-if="item.ckenck == true">
            <!-- 二级功能区 -->
            <div class="buttons">
              <el-button type="primary" class="btnsPrimary" plain @click="gotoNext(0)">提交下一步</el-button>
              <el-button type="danger" class="btnsPrimary" plain @click="regression">回退</el-button>
            ...
            <div class="pot">
              ...
              <ul class="cB_listTitle">
                ...
                <li class="trademark2_1">公章</li>
                <li class="trademark2_2">主检人</li>
              ...
              <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange" class="bordeLine">
                <div class="cB_theCirculation" v-for="(item,index) in dataListTwo" :key="item.id">
                  <ul class="onecB_listcenter" :style="flowColor(item.flowStatus)">
                    ...
                    <li class="trademark2_2">{{item.trademark}}</li>
                    <li class="operation2">
                      <span class="spanclass" @click="circulation(item)">流转</span>
                      <i>&nbsp;</i>
                      <span class="spanclass" @click="goUpload(item)">上传</span>
                      <i>&nbsp;</i>
    ...
    <div :class="{'lxt_block':myStyles==false}">
    ...
    <!-- 弹出 -->
    <el-dialog title="查看" :visible.sync="dialogFormVisible2" :fullscreen="true">
      <!-- 任务下达模块引入 -->
  ...
  export default {
    ...
    methods: {
      taskDetail(item) {  //详情的组件传值
        ...
        this.$api.lxtinitial.detailed(data.taskid, data.searchdate, data.processInstanceId).then(res => {
      ...
      gotoNext(tofinish) { //提交下一步  直接完成
        ...
        if (_list.length == 0) {
          ...
          //判断是否有回退的信息
          let flowkeys = this.statusbox.filter((val, i) => {
            return val.flowStatus != 0
          }).length
          if (flowkeys == 0) { //没有回退的信息
          ...
          this.$alert('请先确认选择了当前选中的公章信息', '信息提示', {
            dangerouslyUseHTMLString: true
          });
        }
      },
