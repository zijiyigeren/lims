  ...
  export default {
    ...
    methods: {
      ...
      dataListData() {//一级列表展示数据
        this.dataList = [];
        //alert(this.querydataChild.length)
      ...
      regression() { //回退按钮  弹出
        if (this.itemValues.length > 0) {
          ...
          beStr = ivSet.join(",");//检测项目id
          this.$api.lxtinitial.goBackClear(beStr).then(res => {   //回退（退回弹框里面的内容信息）之前 调用清除缓存接口
            let data = res.data
            if (data.ok) {
      ...
      look(item) {  //查看报告
        const datatin = {
          ...
          taskId: this.towCurItem.mid,
          sample: item.sample

        }
        // console.log(datatin)
        this.clickOpenReport(datatin)
        // this.$api.lxtinitial.examination(datatin.id,datatin.process_instance_id,datatin.taskId).then(res => {
      ...
      clickOpenReport(row) { // 查看-打开报告
        ...
        if (row.process_instance_id == "5" || row.process_instance_id == "6") {
          // this.goOpenWord(row.id, row.sample, row.process_instance_id);
          this.$api.lxtinitial.examination(row.id, row.process_instance_id, row.taskId).then(res => {
