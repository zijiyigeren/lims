<template>
  <div class="div">
    <!-- <ul class="operate-list">
      <li v-for="(item,ind) in operatelist" :key="ind" @click="onClickLi(item.id)">
    ...
    <div class="wrap">
      ...
      <el-tabs v-model="activeName" type="card" :before-leave="changeTab">
        ...
        <!-- <el-tab-pane label="费用信息" name="sixth">
          <priceInfo :tinfo="information.tinfo" :fitem="information.fitem" ref="priceInfo"></priceInfo>
        </el-tab-pane>-->
      </el-tabs>
    </div>
  </div>
    ...
    saveTask(itype) {
      ...
      if (itype == 0) {
        if (itype == 2) {
          ...
          // copyGO3('itemsSampleDate', 'sampleDate');
          // if (!validateDate()) {
          //   return;
          // }
          // 保存
        ...
        // layer.confirm(message.replace(/\n/g, '<br>'), {
        //   area: ['350px', '500px']
        // }, function () {
        //   document.form1.formaction.value = "tasksend";
        //   if (itype == 2) {

        //   } else {
        //     copyGO3('itemsSampleDate', 'tinfo.sampleDate');
        //     if (!validateDate()) {
        //       return;
        //     }
        //   }
    ...
    * */
    copyCompany() {
      ...
      this.$api.entrust.getCopyMessage(this.taskId.id).then(resp => {
       ...
       // console.log(tinfo,333333333);
       // console.log(infor,4536767);
        ...
        if (tinfo !== null) {
          for (let key in tinfo) {
            ...
            if (that.information.tinfo[key] !== undefined && paramArr.indexOf(key) === -1) {
                // console.log(key)
                 that.information.tinfo[key] = tinfo[key];
               //that.information.tinfo[key] = tinfo[key];
        ...
        if (infor !== null) {
          for (let key in infor) {
            ...
            if ( that.information.infor[key] !== undefined && paramArr.indexOf(key) === -1) {
              if(white_list.indexOf(key)!=-1){
               // console.log(key)
                that.information.infor[key] = infor[key];
              }
            }
          }

        }
        that.information.tinfo.checkRemark2 = infor.checkRemark2;
      }).catch(error => {
        this.$message.error({
          message:error.data.message,
          duration: 1500
    ...
    checkSampleSize(cssArr) {
      ...
      let flag = true;
      let itemCodes = '';
      // this.$api.entrust.checkSampleSize(id).then(resp => {
      //   if (resp.data.data == true) {
      //     flag = false;
      //   }
      // })
