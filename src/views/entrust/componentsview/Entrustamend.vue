  ...
  <div class="entrustamend">
    <div class="warp">
        ...
        <el-form :inline="true" size="small" :model="formInline"
          ...
          <div class="demo-form-inline-input">
            <el-form-item label="修改单号 :">
              <el-input v-model.trim="formInline.code" @keyup.enter.native="searchEnterFun" clearable></el-input>
            </el-form-item>
            ...
            <el-form-item label="负责部门 :">
              <el-input v-model.trim="formInline.dept" @keyup.enter.native="searchEnterFun" clearable></el-input>
            </el-form-item>
            <el-form-item label="报告单号 :">
              <el-input v-model.trim="formInline.itemscode" @keyup.enter.native="searchEnterFun" clearable></el-input>
            </el-form-item>
      ...
      <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible" :before-close="handleDialogClose">
        <el-form :model="formRedact" :inline="true" size="medium" label-position label-width="140px"
                 class="formRedact-style" ref="formRedact">
          <div>
            <el-form-item
              :rules="[
                                { required: true, message: '修改单号不能为空, 请选择', trigger: 'change'}
                            ]"
            >
              <el-input v-model.trim="formRedact.code" size="medium" style="width:150px;marginLeft:138px;"
                        :disabled="true"></el-input>
            </el-form-item>
            <div>
              <el-form-item v-show="codeFlag" label="修改单号：">
                <el-select v-model="code1" style="width:100px;" size="medium" @change="selectCode(code1, 'list')"
                           clearable>
                  <!-- 今年 -->
                  <el-option v-for="item in list"
                             :key="item.id"
                             :label="item.numberType"
                             :value="item.numberType"></el-option>
                </el-select>
                <el-select v-model="code2" style="width:100px;" size="medium"
                           @change="selectCode(code2, 'listLastYear')" clearable>
                  <!-- 去年 -->
                  <el-option v-for="item in listLastYear"
                             :key="item.id"
                             :label="item.numberType"
                             :value="item.numberType"></el-option>
                </el-select>
                <el-select v-model="code3" style="width:100px;" size="medium"
                           @change="selectCode(code3, 'listBeforeYear')" clearable>
                  <!-- 前年 -->
                  <el-option v-for="item in listBeforeYear"
                             :key="item.id"
                             :label="item.numberType"
                             :value="item.numberType"></el-option>
                </el-select>
              ...
              <el-form-item label="类别：">
                <el-select v-model="formRedact.type0" style="width:150px;" size="medium" clearable>
          ...
          <div class="divider">
            <el-form-item label="任务号：">
              <el-input v-model.trim="taskCode" size="medium" style="width:150px" placeholder="请输入内容"></el-input>
            ...
            <div>
              <el-form-item label="搜索结果：">
                <div class='search-results'>
                                    <span v-for="(item,i) in searchResult" :key="i"
              ...
              <el-form-item label="选中结果：">
                <div class="search-results-box">
                  <div class='search-results'>
                                        <span v-for="(item,i) in selectedResults" :key="i"
            ...
            <div class="divider" v-for="(item, i) in reportNumber" :key="i">
              ...
              <div v-for="(em,i) in item.children" :key="i">
                ...
                <el-form-item label="费用：">
                  <el-input v-model.trim="em.reportCost" size="medium" style="width:150px" placeholder="500"></el-input>
                </el-form-item>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      clickXgdEditXgd(title, row) {
        ...
        this.$api.entrustamend.apiXgdFindDetail(params).then(res => {
          if (res.data.ok) {
      ...
      findTaskContentByTaskCode(taskCode) {
        ...
        this.$api.entrustamend.apiTaskAjaxFindTaskContentByTaskCode(params).then(res => {
          if (res.data.ok) {
      ...
      findTaskInfoByCode(taskCode) {
        ...
        if (taskCode) {
          ...
          this.$api.entrustamend.apiTaskAjaxFindTaskInfoByCode(params).then(res => {
            if (res.data.ok) {
              let data = res.data.data;
              ...
              this.formRedact.companyContact = data.companyContact != 'null' ? data.companyContact : '';
      ...
      submitFormVisible(formRedact) {
        ...
        } else if (this.putinStatus == 'redact') { // 编辑
          formRedact.deptname = [];
          formRedact.reportCount = [];
          formRedact.report = [];
          formRedact.trade = [];
      ...
      onClickXgdReport(row) {
        ...
        if (xd2Ids.length > 0) {
          ...
          this.$api.entrustamend.apiXgdReport(params).then(res => {
            this.$message({ type: 'success', message: res.data.data, duration: 1500 })
            this.findList();
      ...
      onClickXgdSendReport(row) {
        ...
        if (xd2Ids.length > 0) {
          ...
          this.$api.entrustamend.apiXgdSendReport(xd2Ids).then(res => {
            this.$message({ type: 'success', message: res.data.data, duration: 1500 })
            this.findList();
            this.clickXgdajaxGetxgl();
          }).catch((error) => {
